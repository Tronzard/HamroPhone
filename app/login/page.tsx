...
function LoginForm() {
  ...
  useEffect(() => {
    if (error === "expired_token") setMessage("Your verification link has expired. Please sign up again.");
    else if (error === "invalid_token") setMessage("Invalid verification link.");
    else if (error === "server_error") setMessage("Something went wrong. Please try again.");
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-4 relative overflow-hidden">
      ...
      >
        ...
        {/* Card */}
        <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl space-y-5">
          ...
          {verified && (
            <motion.div
              ...
              className="flex items-start gap-3 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm px-4 py-3 rounded-xl"
            >
          ...
          <p className="text-center text-white/40 text-sm pt-2">
            ...
            <Link href="/signup" className="text-[#00E5FF] font-semibold hover:underline">
              Create one
            </Link>
          </p>
        </div>
